...
let rand = new Math.seedrandom(window.context.owner);
...
let visPos = [];
let pos = [];
let totalAve = new THREE.Vector3();
...
function init() {
  ...
  //ILLUMINGATED CAMERA
  const pointLight = new THREE.PointLight( 0xFBFAF5, intensityBoost, 300 );
...
function initLights(scene) {
    ...
    const light1 = new THREE.HemisphereLight( 0xffffff, 0xD9D9D9 );
    light1.position.set( - 50, 70, 50 );
    scene.add( light1 );

    const light2 = new THREE.HemisphereLight( 0xffffff, 0xD9D9D9, 0.5 );
    light2.position.set( - 50, - 70, - 50 );
    scene.add( light2 );

    /*
      const light3 = new THREE.PointLight( 0xFBFAF5, 2, 300 );
    light3.position.set( 100, 100, 100 );
    light3.color = light3.color.convertSRGBToLinear();
    scene.add( light3 );

    const light4 = new THREE.PointLight( 0xFBFAF5, 1, 300 );
